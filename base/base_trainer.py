...
class BaseTrainer:
    ...
    """
    def __init__(
        self, model, loss, metrics, resume, config, train_logger=None
    ):
        self.config = config
        ...
        for epoch in range(self.start_epoch, self.epochs+1):
            ...
            for key, value in result.items():
                if key == 'metrics':
                    ...
                    for i, metric in enumerate(self.metrics):
                        log['val_' + metric.__name__] = \
                            result['val_metrics'][i]
                else:
                    log[key] = value
            ...
            if (
                (self.monitor_mode == 'min'
                 and log[self.monitor] < self.monitor_best) or
    ...
    def _save_checkpoint(self, epoch, log, save_best=False):
        ...
        :param save_best: if True,
         rename the saved checkpoint to 'model_best.pth.tar'
        """
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        self.logger.info(
            f"Checkpoint '{resume_path}' (epoch {self.start_epoch}) loaded"
        )
